    ...
    if (!isset($_SERVER['PHP_AUTH_USER'])) {
        ...
        if (($_SERVER['PHP_AUTH_USER'] == "Admin") && ($_SERVER["PHP_AUTH_PW"] == "PassAdmin")) {
            ...
            if (isset($_POST['id'])
                ...
                $db = new Operations();
                if ($db->UpdateUser($_POST['id'],$_POST['Name'], $_POST['Firstname'], $_POST['Password'])) {
                    $reponse['error'] = false;
                    $reponse['message'] = "Un utilisateur a ete modifie  ! ";
                } else {
                    $reponse['error'] = true;
                    $reponse['message'] = "l utilisateur n a pas pu etre modifie il y a peut etre une erreur ";
                }
            } else {
                $reponse['error'] = true;
                $reponse['message'] = "Veuillez bien entrer les infos(Id,Name,Firstname,Password) sinon ca ne marchera pas ";
            }
        } else {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.04 401 Unauthorized");
