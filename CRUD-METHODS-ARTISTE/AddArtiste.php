    ...
    if(!isset($_SERVER['PHP_AUTH_USER'])){
        ...
        if(($_SERVER['PHP_AUTH_USER']=="Admin") && ($_SERVER["PHP_AUTH_PW"]=="PassAdmin")){
            ...
            if (isset($_POST['nom'])
                ...
                $db= new Operations();
                if($db->AddArtiste($_POST['nom'],$_POST['prenom'],$_POST['nomArtiste'],$_POST['age'],$_POST['sexe'],$_POST['style'],$_POST['photo'])){
                    $reponse['error']=false;
                ...
                $reponse['error']=true;
                $reponse['message']="Veuillez bien entrer les infos(nom,prenom,nomArtiste,age,sexe,style et photo) sinon ca ne marchera pas ";
            }
        }else{
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.04 401 Unauthorized");
